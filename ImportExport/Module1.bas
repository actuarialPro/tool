...
'------------------------------------------------------------------------------
' Main entry point ¡X run from a button or macro list
...
Sub RunImportExport(ByVal mode As String)
    ...
    Dim autoSaveResponse As VbMsgBoxResult
    Dim autoSaveAndClose As Boolean
    
    autoSaveAndClose = False
    ...
    '==================================================
    ' STEP 1 ¡X Scan Import sheet; verify files exist
    '==================================================
    ...
    For Each c In wsImport.UsedRange.Cells
        ...
        If c.HasFormula Then
            ...
            If IsExternalReference(formulaText) Then
                ...
                ' Dir() returns "" when the file is already open ¡X check both
    ...
    '==================================================
    ' STEP 2 ¡X Ask user to confirm before writing & Autosave Check
    '==================================================
    ...
    ' Ask for autosave/close ONLY for value and formula export modes
    If mode = "exportValue" Or mode = "exportFormula" Then
        autoSaveResponse = MsgBox( _
            "Do you want to automatically save and close the external workbooks after the update?", _
            vbYesNo + vbQuestion, _
            "Autosave Options")
            
        If autoSaveResponse = vbYes Then
            autoSaveAndClose = True
        End If
    End If
    
    '==================================================
    ' STEP 3 ¡X Backup Import and Export sheets (values only)
    '==================================================
    ...
    '==================================================
    ' STEP 4 ¡X Open linked workbooks that are not open yet
    '==================================================
    ...
    '==================================================
    ' STEP 5 ¡X Copy Export values into external cells
    '==================================================
    ...
    '==================================================
    ' STEP 6 ¡X Optional Autosave & Close
    '==================================================
    If autoSaveAndClose Then
        For Each key In workbookList.Keys
            Set targetWb = WorkbookFromFullPath(CStr(key), workbookList)
